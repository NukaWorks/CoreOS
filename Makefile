...
gccbuild:
	...
	@if [ -f mpfr-4.2.0.tar.xz && ! -f mpfr ]; then tar -xf mpfr-4.2.0.tar.xz && mv -v mpfr-4.2.0 mpfr; else echo "mpfr-4.2.0.tar.xz not found."; fi
	@if [ -f gmp-6.2.1.tar.xz && ! -f gmp ]; then tar -xf gmp-6.2.1.tar.xz && mv -v gmp-6.2.1 gmp; else echo "gmp-6.2.1.tar.xz not found."; fi
